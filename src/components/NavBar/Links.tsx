...
export default function Links({ className }: IClassName) {
    return (
        <ul className={`hidden lg:flex text-text2 font-bold text-sm  gap-5 justify-between  transition-all duration-1000 ${className}`}>
            ...
            <li><Link to="/" className="transition-colors ease-in-out duration-300 hover:text-white">Home</Link></li>
            ...
            <li><Link to="/browse-movies" className="transition-colors ease-in-out duration-300 hover:text-white">Browse Movies</Link></li>
        </ul>
    )
}